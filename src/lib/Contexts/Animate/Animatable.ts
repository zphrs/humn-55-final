...
export function createAnimationInfo<Init extends RecursiveAnimatable<unknown>>(
	...
	const [boundsAnim, boundsChildren] = separateChildren(bounds ?? {})
	...
	for (const [key, child] of Object.entries(children)) {
		;(info.children as any)[key] = createAnimationInfo(
...
export function updateAnimationInfo<Animating extends RecursiveAnimatable<unknown>>(
	...
	for (const childInfo of Object.values<AnimationInfo<RecursiveAnimatable<unknown>>>(
		info.children as { [s: string]: AnimationInfo<RecursiveAnimatable<unknown>> }
	)) {
...
export function changeInterpFunction<Animating extends RecursiveAnimatable<unknown>>(
	info: AnimationInfo<Animating>,
	interp: Interp,
	mask: Partial<Mask<Animating>> = {} // assumes default of true
) {
	...
	for (const childInfo of filteredChildren) {
		changeInterpFunction(
			childInfo[1] as AnimationInfo<Animatable>,
			interp,
			mask[childInfo[0] as keyof typeof mask] as Partial<Mask<Animatable>>
		)
	}
}
