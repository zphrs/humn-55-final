...
export type OptionalFlattenedDictTree<T> = RecursivePartial<FlattenedDictTree<T>>

export function flattenDictTree<T>(tree: OptionalDictTree<T>): FlattenedDictTree<T> {
...
export function unflattenDictTree<T>(tree: FlattenedDictTree<T>): OptionalDictTree<T> {
...
export function flatDictTreeGet<T>(tree: FlattenedDictTree<T>, ...key: string[]): T | undefined {
	return tree[key.join('.')]
}

export function flatDictTreeSet<T>(tree: FlattenedDictTree<T>, key: string[], value: T) {
...
export function dictTreeSet<T>(tree: OptionalDictTree<T>, key: string[], value: T) {
	let val = tree
	for (const k of key.slice(0, key.length - 1)) {
		...
		let currVal = val[k as keyof typeof val]
		if (typeof currVal !== 'object') {
			throw new Error(`Too many keys, reached a leaf at key ${k} out of ${key.length} keys`)
		}
...
export function recursiveCopy<Tree extends OptionalDictTree<unknown>>(obj: Tree) {
	const cpy: Tree = {} as Tree
	for (const key in obj) {
		if (typeof obj[key] === 'object') {
			cpy[key] = recursiveCopy(obj[key] as OptionalDictTree<unknown>) as Tree[Extract<
				keyof Tree,
				string
			>]
		} else {
...
export function mergeDictTrees<T extends unknown, Tree extends OptionalDictTree<T>>(
	oldPartial: Tree,
	newPartial: Tree
): Tree {
	const cpy: Tree = {} as Tree
...
export function* dictTreesToGenerator<List extends OptionalDictTree<unknown>[]>(...args: List) {
	// iterate first arg
	const first = args[0]
